...
Y = np.load("imgBlk.npy")
Z = np.load("segTrn.npy")

model = dmnet()
print model.summary()
# model.load_weights('unet_membrane.hdf5')
model_checkpoint = ModelCheckpoint('dmnet_membraneBlk.hdf5', monitor='loss',verbose=1, save_best_only=True)
tbCallBack = TensorBoard(log_dir='./Graph', histogram_freq=0, write_graph=True, write_images=True)
model.fit([X, Y], Z, epochs=10, batch_size= 5, callbacks=[model_checkpoint,tbCallBack])# ,validation_data=imageLoaderV(fileList1, 2),validation_steps=578)
